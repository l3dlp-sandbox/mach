...
fn renderFieldName(writer: anytype, operands: []const g.Operand, field_index: usize) !void {
    ...
    derive_from_kind: {
        ...
        for (name) |c| {
            switch (c) {
                'a'...'z', '0'...'9' => try name_buffer.appendBounded(c),
                'A'...'Z' => try name_buffer.appendBounded(std.ascii.toLower(c)),
                ' ', '~' => try name_buffer.appendBounded('_'),
    ...
    for (operand.kind, 0..) |c, i| {
        switch (c) {
            ...
            'A'...'Z' => if (i > 0 and std.ascii.isLower(operand.kind[i - 1])) {
                try name_buffer.appendSliceBounded(&[_]u8{ '_', std.ascii.toLower(c) });
            } else {
                try name_buffer.appendBounded(std.ascii.toLower(c));
            },
